...
// Interfeyslar orqali aniq turlarni belgilash
interface Task {
    text: string;
    textUz: string;
    userAnswer?: string;
...
// Propsni aniq tur bilan e’lon qilish
...
const checkAnswer = (index: number) => {
    ...
    if (!exercise) return; // Agar mashq bo'lmasa, hech narsa qilmaymiz
    ...
    // Hammasi to'g'ri ekanligini tekshiramiz
    isCompleted.value = props.lesson?.tasks?.every((ex: Task) => ex.isCorrect) ?? false;
};
...
<template>
    <div
        ...
        <div class="grid md:grid-cols-2 grid-cols-1 p-4 w-[90%] m-auto gap-20">
            <div v-if="courseStore.lessonLoading">Loading...</div>
            <div v-else v-for="(item, index) in props.lesson?.tasks" :key="index">
                ...
                <Input v-if="!item.isCorrect && !props.lesson?.isCompleted" v-model="item.userAnswer" class="mt-2"
                ...
                <button v-if="!item.isCorrect && !props.lesson?.isCompleted" @click="checkAnswer(index)"
                    class="px-2 bg-blue-600 py-1 mt-2 rounded-[7px]">
                    Tekshirish
                </button>

                <span v-if="(item.checked && item.isCorrect) || props.lesson?.isCompleted"
                    class="text-green-500 block mt-1">
                    Javob: {{ item.textUz }}
                </span>
            </div>
        </div>

        <button v-if="isCompleted && !props.lesson?.isCompleted" @click="saveResults"
            class="flex justify-end px-2 bg-blue-600 py-1 rounded-[7px]" :disabled="isLoading">
