            ...
            <template v-else>
                ...
                <template v-else>
                    <form @submit.prevent="handleCreate()">
                        <table class="max-sm:text-[14px] w-full max-xl:min-w-[360px] text-sm shadow-md sm:rounded-lg overflow-hidden text-left rtl:text-right text-gray-500 dark:text-gray-400">
                            ...
                            <tbody>
                                <tr v-for="(item, index) in filteredItems" :key="index" @click="showModal(item.attendance.status)" class="font-medium bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 text-gray-800 dark:text-white">
