                ...
                <form @submit.prevent="handleSubmit()">
                    <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                        <div class="w-full">
                            ...
                            <input type="text" v-model="userData.name" name="brand" id="brand"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-green-500 dark:focus:border-green-500"
                                placeholder="O'qtuvchi ismni kiriting" required="">
                        </div>
                        <div class="w-full">
                            <label for="item-weight"
                                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Guruh
                                darajasi</label>

                            <Popover class="dropdown dropdown-bottom w-full">
                                <PopoverTrigger tabindex="0" role="button"
                                    class="w-full text-white bg-blue-700 hover:bg-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center flex justify-between items-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 border-0">
                                    ...
                                    <ul class="space-y-1">
                                        <li v-for="(group, index) in groupsDegres" :key="index"
                                            ...
                                            <a class="flex items-center w-full cursor-pointer">
                                                <input type="radio" v-model="userData.degree" :value="group"
                                                    :checked="userData.degree === group" :id="group" name="radio-4"
                                                    class="radio radio-accent bg-transparent text-green-600 border-gray-300 focus:ring-2 focus:ring-green-500 transition-all duration-200" />
                                                <label :for="group"
