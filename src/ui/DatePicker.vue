<template>
    <div class="relative z-30 max-md:w-full" ref="datepickerContainerRef">
        <!-- input -->
        <div class="relative flex items-center">
            ...
            class="w-full rounded-lg border border-stroke bg-transparent py-1.5 pl-[50px] pr-8 text-dark-2 outline-none transition focus:border-primary dark:border-dark-3 dark:text-dark-6 dark:focus:border-primary"
        ...
        <div
            ...
            class="shadow-datepicker absolute mt-2 rounded-xl border border-stroke bg-white pt-5 dark:border-0 dark:bg-gray-900 box-shadow"
        >
            <div class="flex items-center justify-between px-5 ">
                ...
                <div
                    id="currentMonth"
                    class="text-lg font-medium text-dark-3 dark:text-white"
                >
                ...
                >
                    ...
                    <div
                    id="days-of-week"
                    class="mb-4 mt-6 grid grid-cols-7 gap-2 px-5"
                    >
                        <div class="text-center text-sm font-medium text-secondary-color">Yak</div>
                        <div class="text-center text-sm font-medium text-secondary-color">Du</div>
                        <div class="text-center text-sm font-medium text-secondary-color">Se</div>
                        <div class="text-center text-sm font-medium text-secondary-color">Cho</div>
                        <div class="text-center text-sm font-medium text-secondary-color">Pay</div>
                        <div class="text-center text-sm font-medium text-secondary-color">Jum</div>
                        <div class="text-center text-sm font-medium text-secondary-color">Sha</div>
                    </div>
                    <!-- kunlar -->
                    <div
                    ref="daysContainer"
                    id="days-container"
                    class="mt-2 grid grid-cols-7 gap-2 px-5"
                    >
                    <!-- Days will be rendered here -->
                        <div
                            v-for="day in days"
                            :key="day"
                            class="flex items-center justify-center cursor-pointer  w-[44px] h-[44px] text-gray-200 dark:text-dark-6 rounded-full hover:bg-green-500 hover:text-white"
                            :class="{ 'bg-green-500 text-white dark:text-white': isSelected(day) }"
                ...
                <div class="mt-5 flex justify-end space-x-2.5 border-t border-stroke p-5 dark:border-dark-3">
                    ...
                    <button
                        id="applyBtn"
                        class="rounded-lg bg-green-500 px-5 py-2.5 text-base font-medium text-white hover:bg-green-500"
                        @click="handleApply"
                    >
                        Tanlash
                    </button>
...
const daysContainerRef = ref(null);
...
let daysArray = Array(firstDayOfMonth).fill(null).map((_, i) => i + 1);
...
return selectedDate.value && selectedDateObj.getDate() === day;
};
...
watch(() => isCalendarOpen.value, (newValue) => {
if (newValue) {
...
<style>
.shadow-datepicker {
    box-shadow: 0 0 15px 0 #cccccc1e;
}
</style>
